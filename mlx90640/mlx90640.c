...
sl_status_t mlx90640_DumpEE(uint16_t *eeData)
{
    uint16_t *data = (uint16_t *)malloc(832 * sizeof(uint16_t)); // Heap allocation
    if (data == NULL) {
        return SL_STATUS_FAIL; // Handle allocation failure
    }

    // Perform EEPROM dump (replace i2cRead with actual implementation)
    sl_status_t status = mlx90640_I2CRead(0x2400, 832, data);
    ...
    memcpy(eeData, data, 832 * sizeof(uint16_t));
...
void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
  ...
  int accRow[24];
  int accColumn[32];
  float *alphaTemp = (float *)malloc(768 * sizeof(float)); // Allocate on heap
  if (alphaTemp == NULL) {
  ...
  free(alphaTemp); // Free memory after use
}
...
{
    float *ktaTemp = (float *)malloc(768 * sizeof(float)); // Allocate on heap
    if (ktaTemp == NULL) {
    ...
    for (int i = 0; i < 24; i++) {
        for (int j = 0; j < 32; j++) {
            ...
            ktaTemp[p] /= pow(2, (double)ktaScale1);
        }
    }
    ...
    free(ktaTemp); // Free the dynamically allocated memory
}
...
{
  float *kvTemp = (float *)malloc(768 * sizeof(float)); // Allocate on heap
  if (kvTemp == NULL) {
      printf("\r\n kvTemp[768] Handle memory allocation failure!\n");
      return;
  }
  int p = 0;
  int8_t KvT[4];
